...
df['bicho_id'] = df['bicho_id'] - 1  # Ajusta índice 0-24
...
# --- 4. Simulação detalhada com limite de 1 mês ---
def simular_apostas_realista_1mes(model, data, N=10, capital_inicial=1000, meta=2000, aposta_unit=150):
    bichos = data['bicho_id'].values
    datas = data['data'].values
    ...
    data_inicio = datas[N]  # data da primeira aposta possível (após N sequências)
    limite_mes = pd.Timestamp(data_inicio) + pd.DateOffset(months=1)

    for i in range(len(bichos) - N):
        data_atual = datas[i+N]
        if data_atual > limite_mes:
            print(f'Limite de 1 mês atingido na rodada {rodada} (data {data_atual}). Parando simulação.')
            break
        ...
        print(f"Rodada {rodada} ({pd.to_datetime(data_atual).date()}): Apostou nos bichos {bichos_para_apostar}, resultado: {real}, "
...
historico_df = simular_apostas_realista_1mes(model, df, N=N, capital_inicial=1000, meta=2000, aposta_unit=1)

# --- 6. Exportar histórico para CSV ---
historico_df.to_csv('historico_apostas_1mes.csv', index=False)
print('Histórico salvo em historico_apostas_1mes.csv')

# --- 7. Plotar evolução do capital ---
plt.figure(figsize=(12,6))
plt.plot(historico_df['rodada'], historico_df['capital_apos'], marker='o')
plt.title('Evolução do Capital nas Rodadas (1 mês)')
plt.xlabel('Rodada')
